...
import org.pizza.pay.http.HttpRequestTemplate;
...
public interface PayService<PC extends PayConfigStorage> {
     ...
     * @return 返回支付方下载对账单的结果
     */
    <T> T downloadbill(Date billDate, BillType billType, Callback<T> callback);
     ...
     *
     * @param handler 消息处理器
     *                配合{@link  org.pizza.pay.api.PayService#payBack(Map, InputStream)}进行使用
     *                <p>
     *                默认使用{@link  org.pizza.pay.api.DefaultPayMessageHandler }进行实现
     */
